...
void Lasersim::TurtleListener::poseCallback(const geometry_msgs::PoseStampedConstPtr &msg)
{
    ...
    try{
        ...
        ROS_WARN("WaitForTransform exception\"%s\" to \"%s\": %s",  
            std::string(v.at(0)+std::string("_base_link")).c_str(), 
            std::string(parent.turtlename + std::string("_")+parent.lasername).c_str(), 
            ex.what());
    }
...
Lasersim::Lasersim(ros::NodeHandle &n): nh(n), 
    turtlelisteners{TurtleListener(*this,"stampedpose1", n), 
        TurtleListener(*this,"stampedpose2", n), 
        TurtleListener(*this,"stampedpose3", n), 
        TurtleListener(*this,"stampedpose4", n)}, //up to 4 turtles
...
void Lasersim::wallsCallback(const sensor_msgs::PointCloudConstPtr &msg)
{
    ...
    try{
       ...
       ROS_WARN("Received an exception trying to transform a point from \"%s\" to \"%s\": %s", 
            msg->header.frame_id.c_str(), 
            std::string(turtlename + std::string("_")+lasername).c_str(), 
            ex.what());
    }
